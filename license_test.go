...
func TestLicenseReading(t *testing.T) {
	...
	assert(license.Seed == 879062772, "Bad seed")
	assert(license.Owner == "Palette Agent DEV", "Bad Owner")
	assert(license.CoreCount == 64, "Bad corecount")
	assert(license.LicenseId == "palette-dev", "Bad licenseId")
	assert(bytes.Equal(license.Token, DEBUG_TOKEN), "Bad token")
}
